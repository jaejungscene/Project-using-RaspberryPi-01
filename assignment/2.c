...
int main()
{
  ...
  if (-1 == GPIOExport(OUT_TO_BUTTON) || -1 == GPIOExport(IN_FROM_BUTTON))
    return(1);
  //Set GPIO directions
  if (-1 == GPIODirection(OUT_TO_BUTTON, OUT) || -1 == GPIODirection(IN_FROM_BUTTON, IN))
    return(2);
  if ( -1 == GPIOWrite (OUT_TO_BUTTON, 1))
    return(3);

  printf("button wait...\n");
  do{
    value = GPIORead(IN_FROM_BUTTON);
    if(value == 0 && prev == 1){ //button의 (value)값이 1에서 0으로 될 때 <- button을 !!!press하는 순간에!!!
      break;
    }
    prev = value;
  } while(1);
